...
- `Namespace` `(string: "")` <EnterpriseAlert inline /> - Specifies the namespace of
  the target service. This takes precedence over the `ns` query parameter,
...
- `ns` `(string: "")` <EnterpriseAlert inline /> - Specifies the namespace in which to request the leaf certificate.
  The namespace can also be specified [through other methods](#methods-to-specify-namespace).
